...
export type ActiveActionId = 'forge_strike';
 ...
 *  - buff: an activatable timed SELF-buff (temporary stats).
...
export type SkillEffect =
  ...
  | { kind: 'active'; cooldownMs: number; action: ActiveActionId }
  | { kind: 'buff'; cooldownMs: number; durationMs: number; stats: SkillStatMods }
  | { kind: 'debuff'; cooldownMs: number; durationMs: number; radius: number }
  | { kind: 'transformation'; cooldownMs: number; durationMs: number; stats: SkillStatMods; tint: number };
...
const BLACKSMITH: ClassSkills = {
  ...
  skills: [
    // --- PASSIVE test: a flat-percentage max-HP boost (proves passive stats). ---
    {
      id: 'bs_test_hp',
      tree: 'defense',
      name: '[TEST] Ironhide',
      description: '+15% maximum HP while unlocked. (test: PASSIVE stat boost)',
      cost: 1,
      tier: 0,
      effect: { kind: 'passive', stats: { maxHPMult: 0.15 } },
      test: true,
    },
    // --- TRANSFORMATION test capstone: timed boost + tint, behind the passive. ---
    {
      id: 'bs_test_crystal',
      tree: 'defense',
      name: '[TEST] Crystal Form',
      description:
        'Activate: become crystal for 8s — +50% damage, +30% damage reduction, +15% speed, and a crystalline glow. (test: TRANSFORMATION capstone)',
      cost: 1,
      prereq: 'bs_test_hp', // tiered toward the capstone
      tier: 1,
      effect: {
        kind: 'transformation',
        cooldownMs: 30000,
        durationMs: 8000,
        stats: { damageMult: 0.5, damageReduction: 0.3, moveSpeedMult: 0.15 },
        tint: 0x66e0ff,
      },
      test: true,
    },
...
/** Combine a list of stat-mod blocks into one aggregate (sums each field). */
export function combineMods(mods: readonly SkillStatMods[]): SkillStatMods {
  const out: Required<SkillStatMods> = { flatMaxHP: 0, maxHPMult: 0, damageMult: 0, moveSpeedMult: 0, damageReduction: 0 };
  for (const m of mods) {
